...
class BookService:
    ...
    @staticmethod
    def get_all_books(page: int = 1, per_page: int = 20, search: str = None) -> Dict[str, Any]:
    ...
    def update_book(book_id: int, data: Dict[str, Any]) -> Book:
        ...
        allowed_fields = ['title', 'author', 'isbn', 'year', 'description', 'total_copies']
    ...
    def get_overdue_loans() -> List[BookLoan]:
        ...
        return BookLoan.query.filter(
            BookLoan.is_returned == False,
            BookLoan.due_date < datetime.utcnow()
        ).all()
