...
The functions in this module allow users to add new users and books to a global library storage,
check if a user with a given username exists in the storage, retrieve the password for a user,
and retrieve a list of dictionaries representing all the users and books in the storage.

Example usage:

    add_user_to_library({"name": "Alice", "email": "alice@example.com", "password": "password123"})
    add_book_to_library({"title": "Harry Potter and the Philosopher's Stone", "author": "J.K. Rowling", "year": 1997})
...
def user_exists(username: str) -> bool:
    """Checks if a user with the given username exists in the global library storage."""
...
def retrieve_password(username: str) -> Optional[str]:
    """Retrieves the password for the user with the given username, if it exists."""
    for user in GLOBAL_STORAGE.users:
        if user.name == username:
...
def read_all_users() -> List[Dict[str, str]]:
    """Returns a list of dictionaries representing all the users in the global library storage."""
    return [asdict(user) for user in GLOBAL_STORAGE.users]


def read_all_books() -> List[Dict[str, str]]:
    """Returns a list of dictionaries representing all the books in the global library storage."""
